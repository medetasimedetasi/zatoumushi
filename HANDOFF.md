...
**次の作業は§9(マップ変更＋新キャラ「タカラダニ」)。新セッションはまず§9を読む。**
...
## 9. 次にやること(2026-07-13 ユーザー要望。マップ変更＋新キャラ。未着手)

着手前に §2(検証)・§7(流儀)・§8(現状)を必ず確認。各項目は実装後に`__shot`+数値検証。

### ① 巣の移設と常設化
- 巣を(0,1)から**ミニマップど真ん中(2,2)へ移設**(`NEST_COL/NEST_ROW`を2,2に)
- 巣マスにも**敵を配置**(現状は無人。通常どおり敵を出す。d=2相当)
- 巣中央のスポナー(nestNode)を**最初から常設**にする(成体の死骸を食べての解禁をやめ、入場時から巣ノードが存在)。死骸アイテムを残すかは任意判断

### ② 最終拠点の一つ上(4,3)= 増援拠点
- ミニマップで**黄色**表示(新クラス例 `.mm.reinforce`)
- マップ上に**緑色の家**を配置。**アクセスすると子ども20匹**出現(巣の補充とは別系統。1回限りか繰り返し可かは要確認→まず「1回限り」で実装が無難)
- 代わりにこのマスは**敵を多めに**配置してよい
- 注意: (4,3)は現状d7で10人並列陣が出るマス。並列陣を残すか置換か要判断(残す＋緑家、が素直か)

### ③ 砦(4,0)も黄色表示
- ミニマップ右上(4,0=人間の大きい家)も**黄色**に(②と同じ色でよい)

### ④ 新キャラ タカラダニ(赤いダニ。**ユーザー添付の参考画像あり→新セッションで再共有してもらうこと**)
見た目: 鮮やかな赤(朱色)の丸っこいビロード状の胴、脚8本(前脚が長め)。**サイズは子どもの約半分**。
挙動:
- ステージ**中央から出現**し、**ランダムに徘徊**(ステージ内を動き回る意。ステージ間移動ではない想定→要確認)。**速度は子どもより速い**
- 成体・子どもが近づくと**逃げる**
- **倒されるとその場で停止しアイテム化**(死体)
- 子どもが**捕食可**。捕食した子どもは**赤子ども(=fast型。rifleで生まれる赤・速い子と同種)になる**
- **6匹に捕食されると灰色になりアクセス不能**(死体はその場に残る)
- **成体は倒せるが捕食はできない**(倒すだけ)
- 倒してから**2分後に新individualがリポップ**
- 要確認: 出現は全ステージか特定ステージか / 「マップ移動」の解釈(ステージ内徘徊で合ってるか)
