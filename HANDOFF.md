...
- 初期カメラは従来より1段引いた`DEFAULT_ZOOM=1.26`

## 11. SE・音量・公開

- `SE/`配下に item / human / defeat / clear / start。安全なwav名へ整理済み
- 全SEは音量バーとミュートに連動。実効音量=`SFX_GAIN[name] * bgmVol`
- 悲鳴は`human:0.02`。BGM通常時だけ`BGM_GAIN=0.9`。エンディングの100%化は維持
- GitHub Pagesワークフロー: `.github/workflows/pages.yml`。公開対象は`index.html`, `BGM`, `SE`

## 12. ユニーク子ども（群れに各1体）

- companion: 20体で確定。高速・成体の近くを護衛、HP3。死体は残り、成体が低HP時に食べると回復。**死亡後の1分待ちは撤廃**し、次に増えた個体が即候補
- slacker: 30体以上で新生児が20%抽選。成体を無視して徘徊、HP5。死亡後60秒
- assassin: 初回40体、再発30体以上で20%抽選。黒色・最遠敵追跡・ジグザグ回避、HP3。死亡後60秒
- guardian: 同一ステージで60体以上→5体以下を経験し、別ステージで増やした新生児が1回だけ変化。足先ピンク・高速・広域索敵、HP6、再発なし
...
- 地形: 成体CGを2.7倍・青緑半透明・横倒しで流用した巨大死骸。足先を内側へ曲げる。死骸を岩で隙間なく囲み、**画面左向きの細長い隘路だけを入口にする**
